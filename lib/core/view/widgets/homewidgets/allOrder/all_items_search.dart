...
class AllItemsSearch extends StatefulWidget {
  const AllItemsSearch({super.key});

  @override
  State<AllItemsSearch> createState() => _AllItemsSearchState();
}

class _AllItemsSearchState extends State<AllItemsSearch> {
  ...
  Future<void> _loadOrders() async {
    ...
    if (userId != null) {
      ...
      List<Map<String, dynamic>>? cachedOrders =
          await SharedPrefService.getAllOrders();
      if (cachedOrders != null) {
        // Use cached data
        setState(() {
          _orders = cachedOrders;
          _filteredOrders = cachedOrders;
        });
      }
      ...
      body: BlocConsumer<OrderBloc, OrderState>(
        listener: (context, state) {
          if (state is OrderLoaded) {
            ...
            SharedPrefService.saveAllOrders(state.orders);
